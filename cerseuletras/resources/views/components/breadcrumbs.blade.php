...
<nav class="flex mb-4" aria-label="Breadcrumb">
    <ol class="inline-flex flex-wrap items-center gap-y-1 text-sm">
        <li>
            <a href="{{ route('home') }}" class="text-gray-500 hover:text-unmsm-azul transition-colors">
        ...
        @foreach($items as $item)
            <li class="flex items-center">
                ...
                @if(!$loop->last && !empty($item['url']))
                    <a href="{{ $item['url'] }}" class="text-gray-500 hover:text-unmsm-azul transition-colors">{{ $item['label'] }}</a>
                @else
