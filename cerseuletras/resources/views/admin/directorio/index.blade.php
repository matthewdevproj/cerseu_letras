...
@section('content')
    ...
    <div class="md:flex md:items-center md:justify-between mb-8">
        <div class="flex-1 min-w-0">
            ...
            <p class="mt-1 text-sm text-gray-500">Administra el personal del directorio del CERSEU.</p>
        </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($directorio as $persona)
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-900 max-w-xs truncate" title="{{ $persona->cargo }}" aria-label="{{ $persona->cargo }}">{{ $persona->cargo }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm">
                                    @if($persona->correo_persona)
                                        <div class="text-gray-600 flex items-center gap-1">
                                            ...
                                            <span class="truncate max-w-[150px]" title="{{ $persona->correo_persona }}" aria-label="{{ $persona->correo_persona }}">{{ $persona->correo_persona }}</span>
                                        </div>
                                    @endif
                                    @if($persona->anexo)
                                        <div class="text-gray-500 flex items-center gap-1">
                                            <x-fas-phone class="text-xs" />
                                            {{ $persona->anexo }}
                                        </div>
                                    @endif
                            ...
                            <td class="px-4 py-4 whitespace-nowrap text-center">
                                <div class="flex justify-center gap-1">
                                    <form action="{{ route('admin.directorio.moveUp', $persona) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="w-8 h-8 flex items-center justify-center bg-gray-100 text-gray-600 rounded hover:bg-gray-200 transition-colors" title="Subir" aria-label="Subir">
                                            <x-fas-arrow-up />
                                        </button>
                                    </form>
                                    <form action="{{ route('admin.directorio.moveDown', $persona) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="w-8 h-8 flex items-center justify-center bg-gray-100 text-gray-600 rounded hover:bg-gray-200 transition-colors" title="Bajar" aria-label="Bajar">
                                            <x-fas-arrow-down />
                                        </button>
                                    </form>
                                </div>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-3">
                                    <a href="{{ route('admin.directorio.edit', $persona) }}" class="text-blue-600 hover:text-blue-800 transition-colors" title="Editar" aria-label="Editar">
                                        <x-fas-pencil-alt class="text-lg" />
                                    </a>
                                    <form action="{{ route('admin.directorio.toggle', $persona) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="{{ $persona->activo ? 'text-orange-500 hover:text-orange-700' : 'text-green-500 hover:text-green-700' }} transition-colors" title="{{ $persona->activo ? 'Desactivar' : 'Activar' }}" aria-label="{{ $persona->activo ? 'Desactivar' : 'Activar' }}">
                                            <x-dynamic-component :component="'fas-' . ($persona->activo ? 'lock' : 'lock-open')" class="text-lg" />
                                        </button>
                                    </form>
                                    <button type="button"
                                            onclick="window.dispatchEvent(new CustomEvent('confirm-delete', { detail: { action: '{{ route('admin.directorio.destroy', $persona) }}', name: '{{ addslashes($persona->nombre_persona) }}', title: '¿Eliminar registro?' } }))"
                            ...
                            <td colspan="7" class="px-6 py-16 text-center">
                                <div class="flex flex-col items-center">
                                    <div class="w-14 h-14 bg-gray-100 rounded-full flex items-center justify-center mb-4">
                                        <x-fas-address-book class="text-2xl text-gray-400" />
                                    </div>
