            ...
            <div class="relative group h-full flex items-center">
                ...
                @if ($destino)
                    <a href="{{ $destino }}" aria-haspopup="true"
                        ...
                        class="nav-item text-white font-medium hover:text-red-500 transition py-4 flex items-center gap-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-white focus-visible:outline-offset-2 rounded {{ $item->esta_activo ? 'text-red-500 font-bold' : '' }}">
                ...
                <div class="absolute top-full left-0 w-64 bg-white shadow-xl border-t-4 border-red-700 rounded-b-md hidden group-hover:block group-focus-within:block text-gray-700 text-sm z-50">
                    @foreach ($item->hijos as $hijo)
                        ...
                        <a href="{{ $hijo->enlace }}"
                            ...
                            @if ($hijo->icono)
                                <x-dynamic-component :component="$hijo->icono" class="mr-2 text-red-700" aria-hidden="true" />
                            @endif{{ $hijo->etiqueta }}
                        </a>
                    @endforeach
                </div>
            ...
            <div x-data="{ expanded: false }" class="border-b border-gray-50">
                <button @click="expanded = !expanded" type="button" :aria-expanded="expanded.toString()"
                    ...
                    <span class="flex items-center font-semibold text-base">
                        @if ($item->icono)
                            <x-dynamic-component :component="$item->icono" class="w-6 text-center text-red-700/80 group-hover:text-red-700 mr-3 transition-colors" aria-hidden="true" />
                        @endif
                        {{ $item->etiqueta }}
                    </span>
                    <x-fas-chevron-down class="text-xs text-gray-400 group-hover:text-red-700 transition-transform duration-300"
                ...
                <div x-show="expanded" x-collapse class="bg-gray-50/80">
                    ...
                    @if ($destino)
                        <a href="{{ $destino }}" @click="mobileMenuOpen = false"
                            @if ($item->nueva_pestana) target="_blank" rel="noopener noreferrer" @endif
                            class="block px-6 py-3 pl-14 text-gray-700 hover:text-red-700 hover:bg-red-50 text-sm font-semibold transition-colors border-l-2 border-transparent hover:border-red-700">
                    ...
                    @foreach ($item->hijos as $hijo)
                        ...
                        <a href="{{ $hijo->enlace }}" @click="mobileMenuOpen = false"
                            @if ($hijo->nueva_pestana) target="_blank" rel="noopener noreferrer" @endif
                            class="block px-6 py-3 pl-14 text-gray-600 hover:text-red-700 hover:bg-red-50 text-sm font-medium transition-colors border-l-2 border-transparent hover:border-red-700">{{ $hijo->etiqueta }}</a>
                    @endforeach
                </div>
            </div>
        @elseif ($destino)
