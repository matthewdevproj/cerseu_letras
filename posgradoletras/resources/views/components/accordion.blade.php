...
<div class="border-b border-gray-200 last:border-b-0" x-data="{ open: {{ $active ? 'true' : 'false' }} }">
    <button @click="open = !open" type="button"
        ...
        id="{{ $id }}-trigger"
        class="w-full px-6 py-5 bg-gradient-to-r from-white to-gray-50 hover:from-gray-50 hover:to-gray-100 flex items-center justify-between transition-all duration-200 hover:shadow-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-unmsm-guinda"
        :class="{ 'bg-gradient-to-r from-unmsm-guinda/5 to-unmsm-guinda/2 border-l-4 border-unmsm-guinda': open }">
        <h2 class="text-lg font-semibold transition-colors duration-200" :class="{ 'text-unmsm-guinda': open, 'text-gray-900': !open }">
            {{ $title }}
        </h2>
