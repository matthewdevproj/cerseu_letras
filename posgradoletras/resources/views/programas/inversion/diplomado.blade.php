...
@php
    ...
    // Datos base por módulo (matrícula y créditos)
    $modulos = [
        1 => ['matricula' => 200, 'creditos' => 12],
        2 => ['matricula' => 200, 'creditos' => 12],
    ];

    // Calcular totales
    $costoTotal = 0;
    foreach ($modulos as &$mod) {
        $mod['costoModulo'] = $mod['creditos'] * $costoPorCredito;
        $mod['cuotaMensual'] = $mod['costoModulo'] / 3;
        $costoTotal += $mod['matricula'] + $mod['costoModulo'];
    }
    unset($mod);

    $nombreModulo = [1 => 'Primer', 2 => 'Segundo', 3 => 'Tercer', 4 => 'Cuarto'];
@endphp
...
<div class="space-y-8">
        ...
        <div class="grid sm:grid-cols-2 gap-4">
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <div class="bg-unmsm-guinda text-white text-center py-5">
                    <span class="text-3xl font-bold">S/ 200</span>
                </div>
            ...
            <div class="border border-gray-200 rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                <div class="bg-unmsm-guinda text-white text-center py-5">
                    <span class="text-3xl font-bold">S/ 280</span>
                </div>
                <div class="bg-white text-center py-4 px-4">
                    <p class="text-sm text-gray-600 leading-relaxed">
    ...
    <section class="space-y-4">
        ...
        <p class="text-sm text-gray-700 text-justify">
            Cada curso del diplomado tiene un valor en créditos. Es de acuerdo al
            <span class="font-semibold">número de créditos matriculados</span> que se determina el pago de un participante por
            módulo.
        </p>
    ...
    <section class="space-y-6">
        ...
        <div class="grid md:grid-cols-2 gap-6">
            @foreach($modulos as $num => $datos)
                <div class="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                    <div class="bg-unmsm-guinda text-white px-4 py-3">
                        <h5 class="font-semibold text-center">{{ $nombreModulo[$num] }} módulo</h5>
                    </div>
                    ...
                    <table class="w-full text-sm">
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50">
                                <td class="px-4 py-3 text-gray-600">Costo por módulo</td>
                                <td class="px-4 py-3 text-right font-semibold text-gray-900">
                                    S/&nbsp;{{ number_format($datos['costoModulo'], 0) }}</td>
                            </tr>
                            <tr class="bg-unmsm-guinda/5">
                                <td class="px-4 py-3 text-gray-700 font-medium">Cuota mensual (3 cuotas)</td>
    ...
    <section class="bg-green-50 border border-green-200 rounded-xl p-4 md:p-5 space-y-2">
        ...
        <ul class="list-disc list-inside text-sm text-green-900 space-y-1 text-justify">
            <li>
                El costo por módulo <span class="font-semibold">se divide en tres cuotas</span>,
                lo que da el costo mensual.
            </li>
            <li>
                Si realiza el <span class="font-semibold">pago del módulo completo</span> en el primer mes,
