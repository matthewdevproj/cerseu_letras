    ...
    <div class="container mx-auto px-4 max-w-6xl">
        <div class="text-center mb-12">
            <h2 class="text-3xl md:text-4xl font-serif font-bold text-gray-900 mb-3">
                <x-fas-file-alt class="text-unmsm-azul mr-3" />
        ...
        @if($informativos->count() > 0)
            <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 justify-center">
                @foreach($informativos as $categoria => $items)
                    <a href="{{ route('informativos.index') }}"
                        ...
                        <!-- Decoración de fondo -->
                        <div
                            class="absolute top-0 right-0 w-24 h-24 bg-unmsm-azul/5 rounded-bl-full -mr-4 -mt-4 transition-transform group-hover:scale-150">
                        </div>

                        <div
                            class="w-14 h-14 md:w-20 md:h-20 rounded-2xl bg-unmsm-azul/5 text-unmsm-azul flex items-center justify-center text-2xl md:text-4xl mb-4 md:mb-6 group-hover:bg-unmsm-azul group-hover:text-white transition-colors shadow-sm relative z-10">
                            <x-fa-icon :icon="$items->first()->icono ?? 'fas fa-folder'" />
                        </div>

                        <h3
                            class="text-xl font-serif font-bold text-gray-900 mb-2 group-hover:text-unmsm-azul transition-colors relative z-10">
                            {{ $categoria }}
                        </h3>
                        ...
                        <span
                            class="inline-flex items-center gap-2 text-unmsm-azul font-bold text-sm bg-unmsm-azul/5 px-4 py-2 rounded-full group-hover:bg-unmsm-azul group-hover:text-white transition-all relative z-10">
            ...
            <div class="text-center mt-12">
                <a href="{{ route('informativos.index') }}"
                    class="inline-flex items-center gap-2 bg-white border-2 border-unmsm-azul text-unmsm-azul px-8 py-3 rounded-xl font-bold hover:bg-unmsm-azul hover:text-white transition-all duration-300 shadow-sm hover:shadow-lg">
