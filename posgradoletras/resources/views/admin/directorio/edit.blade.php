...
@section('content')
    ...
    <div class="mb-8">
        <nav class="flex mb-4" aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3">
                ...
                <li>
                    <div class="flex items-center">
                        <i class="ph ph-caret-right text-gray-400 mx-2"></i>
                        <span class="text-gray-700 text-sm font-medium">Editar</span>
                    </div>
                </li>
            </ol>
        </nav>
        ...
        <p class="mt-1 text-sm text-gray-500">Modifica la información del miembro del directorio.</p>
    </div>

    <!-- Form -->
    <div class="bg-white shadow-sm border border-gray-200 rounded-lg">
        <form action="{{ route('admin.directorio.update', $directorio) }}" method="POST" class="p-6 space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="w-full md:w-1/4">
                ...
                <input type="number" name="orden" id="orden" value="{{ old('orden', $directorio->orden) }}" min="0"
                    class="block w-full py-2 px-3 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-gold focus:border-brand-gold sm:text-sm">
