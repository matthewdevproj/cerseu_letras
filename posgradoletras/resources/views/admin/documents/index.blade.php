                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        ...
                                        @endif
                                        <a href="{{ $document->url }}" target="_blank" class="text-xs text-blue-600 hover:underline">Ver archivo</a>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-3">
                                    <a href="{{ route('admin.documents.edit', $document) }}" class="text-blue-600 hover:text-blue-800 transition-colors" title="Editar">
                                        <i class="fas fa-edit text-lg"></i>
                                    </a>
                                    <form action="{{ route('admin.documents.toggle', $document) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" class="{{ $document->published ? 'text-orange-500 hover:text-orange-700' : 'text-green-500 hover:text-green-700' }} transition-colors" title="{{ $document->published ? 'Despublicar' : 'Publicar' }}">
                                            <i class="fas fa-{{ $document->published ? 'eye-slash' : 'eye' }} text-lg"></i>
                                        </button>
                                    </form>
                                    <button type="button"
                                            onclick="openDeleteModal({{ $document->id }}, '{{ addslashes($document->original_name) }}')"
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl max-w-md w-full">
                <div class="p-6 text-center">
                    ...
                    <h3 class="text-xl font-bold text-gray-900 mb-2">¿Eliminar documento?</h3>
...
<script>
    function openDeleteModal(id, nombre) {
        ...
        document.getElementById('deleteModalMessage').innerHTML = '¿Estás seguro de eliminar "<strong>' + nombre + '</strong>"? Esta acción no se puede deshacer.';
