        ...
        <div class="grid sm:grid-cols-2 gap-4">
            @foreach (\App\Models\ContentPage::PAGINAS as $slug => $nombre)
                ...
                <a href="{{ route('admin.contenido.edit', $slug) }}"
                    class="group block bg-white rounded-lg shadow-sm border border-gray-100 p-6 hover:border-unmsm-azul/40 hover:shadow-md transition-all">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            <h2 class="font-bold text-gray-900 group-hover:text-unmsm-azul transition-colors">{{ $nombre }}</h2>
                            <p class="text-sm text-gray-500 mt-0.5">/{{ $slug }}</p>
                        </div>
                        <x-fas-arrow-right class="mt-1 text-gray-300 group-hover:text-unmsm-azul motion-safe:group-hover:translate-x-1 transition-all" aria-hidden="true" />
                    </div>
