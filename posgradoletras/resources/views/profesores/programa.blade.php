...
@section('title', 'Profesores de ' . $programa['titulo'] . ' - CERSEU Letras UNMSM')

@section('content')
    <div class="container mx-auto px-4 py-8">

        <a href="{{ route('cursos.show', $programa['slug']) }}"
            class="text-unmsm-dorado hover:text-unmsm-azul flex items-center gap-2 mb-6 text-sm font-medium transition-colors">
        ...
        <h2 class="text-2xl md:text-3xl font-bold text-unmsm-azul mb-2 font-serif">
            Profesores Asesores
        </h2>
                ...
                @foreach($profesores as $profesor)
                    <div class="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-lg transition-shadow">
                        <div class="flex items-start gap-4 mb-4">
                            <div
                                class="w-20 h-20 bg-unmsm-azul/10 rounded-full flex items-center justify-center text-unmsm-azul shrink-0">
                                ...
                                <ul class="space-y-1">
                                    @foreach($profesor['lineas_investigacion'] as $linea)
                                        <li class="flex items-start gap-2 text-gray-600">
                                            <span class="w-1.5 h-1.5 bg-unmsm-azul rounded-full mt-2 shrink-0"></span>
                                            {{ $linea }}
                                        </li>
                                    @endforeach
                                </ul>
                        ...
                        <div class="flex gap-3 pt-4 border-t border-gray-100">
                            @if($profesor['cti_vitae'])
                                <a href="{{ $profesor['cti_vitae'] }}" target="_blank" rel="noopener noreferrer"
                                    class="inline-flex items-center gap-2 text-sm border border-gray-200 px-4 py-2 rounded hover:bg-unmsm-azul/5 text-gray-600 transition-colors">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @else
            <div class="text-center py-12 bg-white rounded-lg border border-gray-200">
                <p class="text-gray-500">No hay profesores asignados a este curso actualmente.</p>
            </div>
        @endif
    </div>
@endsection
